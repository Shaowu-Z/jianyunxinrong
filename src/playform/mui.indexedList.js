...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
			...
			self.el.newFriend = self.box.querySelector(classSelector('singlebox'));
		},
		caleLayout: function() {
			var self = this;
			if(self.el.newFriend){
			    var withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight - self.el.newFriend.offsetHeight - 10)  + 'px';
			}else{
			...
			var barItemHeight = ((self.el.bar.offsetHeight - 40 ) / (self.el.barItems.length+1)) + 'px';
