...
<div id="select_children" class="mui-page" style="display:none">
	<div class="mui-navbar-inner mui-bar mui-bar-nav">
		<div class="mui-navbar-inner mui-bar mui-bar-nav">
			...
			<h1 class="mui-center mui-title">组织架构333</h1>
            <button class="mui-btn mui-btn-nav mui-btn-primary mui-pull-right mui-disabled ok-btn" @click="trueSelectUser">完成</button>
		</div>
	</div>
  ...
  methods:{
    ...
    //确定选中的userIds
		trueSelectUser: function () {
			...
			if(_self.select_member.selectArr.length==2){
				l = tipApi.load("请稍等...");
				for(var i=0;i<_self.select_member.selectArr.length;i++){
					if(_self.select_member.selectArr[i]!=userId){
						selectUserIdsEvy=_self.select_member.selectArr[i];
					}
				}
			}else{
				l= tipApi.load("创建中，请稍等");
				for(var i=0;i<_self.select_member.selectArr.length;i++){
					if(i==_self.select_member.selectArr.length-1){
						selectUserIdsEvy+=_self.select_member.selectArr[i];
					}else{
						selectUserIdsEvy+=_self.select_member.selectArr[i]+",";
					}
...
//				console.info(response.data.result);
        ...
        createGroup: function (groupName) {
            ...
            var userAvatar = host+"/static/images/ico_groupchat.png";
			var pram = {groupType:"2",userIds:selectUserIds,groupName:groupName,avatar0:userAvatar};//自主创建群组
			_self.$http.post("/concats_api/create_group",pram).then(function (response) {
				console.log(response.data.result.sdkChatId)
				if(response.data.code==200){
                    tipApi.success("群组创建成功",2)
                    setTimeout(function(){
       ...
       selectInArr:function(bool,val) {
        ...
        var count = _self.select_member.selectArr.length;
