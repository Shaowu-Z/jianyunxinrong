...
var calendar_manage={
 ...
 //绘制
  getIndexDay:function() {
    calendar_manage.isLeapYear();
    calendar_manage.getDays();
    riliHtml = '';
    //本月一号几
    ...
    for(var k = 0; k < calendarDate.days; k++) {
        ...
        for(var d in calendarDate.opt.data) {
            ...
            flag = calendar_manage.checkDate(nowDate, dataDate);
            if(flag) {
    ...
    $('.ht-rili-onclick').on('click', function() {
        calendar_manage.dateClick(this);
    })
...
calendar: function(opt) {
    setTimeout(function () {
        ...
        $('.ht-rili-leftarr').on('click', function() {
            ...
            laowu_calendar.getDataByMonthForManage(month);
            if(dirip<mydate){
                for(var i = 0; i < 42; i++) {
                    if($(".ht-rili-body div").eq(i).text() == 1) {
                        ...
                        for(var j = i; j < daytime + i; j++) {
                            ...
                            laowu_calendar.setDataForManage(calendarDate,j);
                        }
                        break
                    }
                }
...
export default calendar_manage;
