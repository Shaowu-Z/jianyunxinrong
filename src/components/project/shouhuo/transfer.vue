...
//											appApi.openNewWindow("/static/newwebstatic/lianxi/work_content.html?id="+this.$route.query.id)
//										}
//									}else{
//										
//									}
...
//											window.location.replace("/static/newwebstatic/lianxi/work_content.html?id="+this.$route.query.id)
										}else{
										...
										history.replaceState(null, null,"newInfo.html?id="+_self.$route.query.id)  //h5  替换当前页面url（手机上url替换但是页面没变）
										 location.replace(''); //加载新url页面
//										window.location.replace("/static/newwebstatic/lianxi/newInfo.html?id="+this.$route.query.id)
									}
								}else{
									 history.replaceState(null, null,"newInfo.html?id="+_self.$route.query.id)  //h5  替换当前页面url（手机上url替换但是页面没变）
									 location.replace(''); //加载新url页面
