...
export default {
    ...
    created: function() {
       var _self = this
        if(_self.$route.query.currUserId!=undefined){  //组合查询寄进来的
            _self.fromcome=1
            _self.currroomid=_self.$route.query.currRoomId
            if(_self.$route.query.currUserId!=""){
            _self.fromUserid=_self.$route.query.currUserId
            }else{
                _self.fromUserid=_self.userid;
            }
            _self.urlcan="&currUserId="+_self.$route.query.currUserId+"&currRoomId="+_self.$route.query.currRoomId
        }
        _self.initData()
