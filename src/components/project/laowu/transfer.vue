...
//									alert(_self.confirm+"***"+_self.roomId+"******"+_self.currroomid+"//"+_self.creatorId+"**"+userid)
									...
									if(_self.roomId==_self.currroomid){//判断当前房间是否是推送的房间
										if(_self.creatorId==_self.userid){ //判断当前人是否是录入人
                                            ...
                                            //  location.replace(''); //加载新url页面
                                              _self.$router.replace({ path: '/static/newwebstatic/laowu/work_content.html', query: { id: _self.$route.query.id}})
//											window.location.replace("/static/newwebstatic/lianxi/work_content.html?id="+_self.$route.query.id)
										}
									}else{
										history.replaceState(null, null,"newInfo.html?id="+_self.$route.query.id)  //h5  替换当前页面url（手机上url替换但是页面没变）
                                        //  location.replace(''); //加载新url页面
                                         _self.$router.replace({ path: '/static/newwebstatic/laowu/newInfo.html', query: { id: _self.$route.query.id}})
//										window.location.replace("/static/newwebstatic/lianxi/newInfo.html?id="+_self.$route.query.id)
									}
								}else{
									 history.replaceState(null, null,"newInfo.html?id="+_self.$route.query.id)  //h5  替换当前页面url（手机上url替换但是页面没变）
                                    //  location.replace(''); //加载新url页面
