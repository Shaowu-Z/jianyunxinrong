						...
						<ul class="mui-table-view publish-box eg-table-view">
							<li class="mui-table-view-cell mui-input-row"><label>本次报量区间</label>
								<div class="input-muitl">
                                    ...
                                    <div style="width:48%;float:left">
                                        <data-bar :title="returntitle1" @datatoParent="childValue1"></data-bar>
									</div>
...
export default {
    ...
    danjuApi.initVue();
					...
					_self.dateShenqing = nowyear + "-" + nowmoth +
            ...
            // this.test=val;
				...
				return t;
					...
					next: function(pageIndex) {
						...
						if(pageIndex == 1) {
							var startdate = parseInt(_self.startDate.split("-")[0] + _self.startDate.split("-")[1] + _self.startDate.split("-")[2])
							var enddate = parseInt(_self.endDate.split("-")[0] + _self.endDate.split("-")[1] + _self.endDate.split("-")[2])
							var nowData=parseInt(_self.nowdata.split("-")[0] + _self.nowdata.split("-")[1] + _self.nowdata.split("-")[2])
//							alert(enddate》nowData)
					...
					getnews: function() {
						...
						this.$http
							.get(
								...
								"/pcontact_api/getroominfo?roomId=" + _self.currRoomId)
							.then(
								function(response) {
									...
									if(response.data.code == 200) {
										...
										$.each(_self.gongdanTabs, function() {
											$.each(_self.value, function() {
												if(_self.selectdStatus) {
													updTabs.push({
														"shuliang": Number(_self.tabs[_self.index].shuliang) + Number(this.withShuliang),
														"sid": _self.sid
													})
												}
											})
										...
										var todojson = {
											"title": decodeURI(username) +
												"的报量",
											"titileTwo": _self.currRoomClassName +
												"-" +
												_self.curRoomName,
											"content": "开始日期=" +
												_self.startDate +
					...
					aa: function(CONTENT) {
						...
						_self.tabs[_self.nuberhang].mingcheng = addname
							.split("$,")[1]
					},
					getBaoliangInfo: function() {
						var _self = this
						this.$http
							.post( "/chart/column/baoliang_info?companyShigongID=" + _self.companyBuyRoomID + "&currRoomId=" + _self.currRoomId) //_self.companyBuyRoomID+
					...
					changRoom: function(n) {
						...
						window.appApi
							.openProjectContactSelectPage(_self.projectSn,
								'', currRoomCreditCode, 3, false)
					},
					moveimg: function(n) {
						var _self = this;
...
.mui-table-view-cell.mui-input-row .input-muitl .f-date, .mui-table-view-cell.mui-input-row .input-muitl .r-date {
    float: left;
    width: 48%;
    font-size: 16px;
}
...
.oa-contact-avatar p, .oa-contact-avatar .oa-pic-default {
    display: block;
    width: 40px;
    height: 40px;
    background: #4ba9e9;
...
.oa-contact-name ~ .oa-contact-email,label{
    text-align: left
}
.container-average {
    overflow: hidden;
