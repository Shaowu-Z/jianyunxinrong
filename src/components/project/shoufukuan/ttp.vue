...
//											"title": decodeURI(username) + "退回的收付款",
											"title": "收付款退回:"+this.$route.query.title+"的收付款",
											"titileTwo": this.$route.query.roomclass + "-" + this.$route.query.roomname,
											"content": "类别=" + this.$route.query.leibie +"|日期=" + _self.printTime,
//											"fileCount": imgid.length+fujianid.length,
											"fileCount":0,
											"url":  + '/static/newwebstatic/shoufukuan/transfer.html?id='+ this.$route.query.id,
											"colorString": "",
											...
											"setButton": [{
												...
												"name": "确认",
												"url": +"/contract/do_todobtu?type=4&pingfen=0&docid="+  _self.$route.query.id+"&projectSn="+ _self.$route.query.projectSn+"&_self.userid="+_self.userid,
//												"pingfen":"",
//												"docid":"",
//												"roomId":"",
//												"roomName":"",
...
//												"_self.userid":""
									...
									} else if(_self.typ == 1) {//										appApi.refreshData(2)
//										window.appApi.closeNewWindow()
										var parm = {
											"toImId": this.$route.query.formroomimid,
											"chatType": "2",
											"toNickName": "",
											"toAvatarUrl": "",
//														myExtType: 'notify_type',
											"content": this.$route.query.roomname + "已确认" + _self.$route.query.fromRoomName + "的收付款",
										}
...
// 						xhr.send(form);
					...
					moveimg:function(n){
						var _self=this;
						this.$http.post("/sass_api/delete_file?_self.userid=_self.userid&fileId=" + _self.zrimg[n-1]).then(function(response) {
...
//						imgid.remove(n-1)
					...
					movefj:function(n){
						var _self=this;
						this.$http.post( "/sass_api/delete_file?_self.userid=_self.userid&fileId=" + _self.zrfujian[n-1]).then(function(response) {
							if(response.data.code == 0) {
								console.log(response.data)
                                // ludan("删除成功",1,2)
                                layer.open({
...
<style>
    ...
    .container-average .img-item .img-item-inner, .publish-container .upload-btn .img-item-inner{
        bottom: 0;
        top: 10px;
    }
