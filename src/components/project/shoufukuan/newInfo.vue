        ...
        <header class="mui-bar mui-bar-nav">
			...
			<div v-for="(n,index) in jieshou" :key="index">
				...
				<div v-if="n.confirmStatus==1">
					<img class="zhuangtai" :src='imgurl' />
                    <!-- <img class="zhuangtai" src="../../../../static/images/approval-by.png" /> -->
				</div>
...
export default {
  data() {
    return {
      ...
      imgurl: "../../../../static/images/approval-by.png",
  ...
  methods: {
    ...
    initdata: function() {
      ...
      this.$http.post("/contract/get_content", param).then(function(response) {
        ...
        if (response.data.code == 200) {
          ...
          // var pinlun=_self.noteList.content.replace(/#.%#/g, '<br/>')"
          ...
          //当前用户房间信息
          _self.usermain(
            response.data.result.flowList,
            JSON.parse(response.data.result.noralJson).table.projectid
          );
          //图片  附件
          //其他信息
    ...
    open_file: function(id) {
      ...
      alert(document.domain + "/sass_api/file/download?id=" + id);
      // appApi.openFile("http://"+window.location.host+'/sass_api/file/download?id=' + id);
    },
    //头像信息
    lookUserInfo: function(userId) {
      //查看用户资料
      window.appApi.openNewWindow(
        getPagePath() + "/contacts/eg_details.html?userId=" + _self.userId
      );
    },
    //查询当前用户信息
    ...
    yes: function() {
      ...
      window.location.href =
        "/static/newwebstatic/shoufukuan/ttp.html?id=" +
        this.$route.query.id +
        "&projectid=" +
        _self.projectid +
        "&roomid=" +
    ...
    pin: function() {
      ...
      // var urlcan=this.$route.query.id+ "&projectid=" + _self.projectid + "&roomid=" + _self.toroomid + "&roomname=" + _self.curoomname + "&typ=3" + "&nowroomImId=" + _self.nowroomImId
      this.$router.push({
