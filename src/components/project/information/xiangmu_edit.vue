        ...
        <section class="mui-content mycenter-content">
            ...
            <ul class="mui-table-view eg-table-view">
                ...
                <li class="mui-table-view-cell mui-input-row">
                    <a class="mui-navigate-right">
                        <label>开工日期</label>
                        <input type="text" name="missionStartDate" v-model="form.missionStartDate" @click="app.selectDate('s')"
                            readonly="readonly"
                            placeholder="请选择">
                    </a>
                </li>
                <li class="mui-table-view-cell mui-input-row">
                    <a class="mui-navigate-right">
                        <label>竣工日期</label>
                        <input type="text" name="missionEndDate" v-model="form.missionEndDate" @click="app.selectDate('e')"
                            readonly="readonly"
                            placeholder="请选择">
                    </a>
                </li>
                ...
                <div class="module01-head">工程概况照片</div>
                    <div class="module01-body">
                        ...
                        <div class="upload-btn mui-text-center">
                            ...
                            <input name="need_hide_div" type="file" id="upfile" value="" onclick="window.webactivity.setInputType(1);" accept="image/png,image/gif,image/jpeg" onchange="selectimgUrl(this)" class="input-file">
                        </div>
                    </div>
...
export default {
    ...
    created() {
        ...
        var urlProjectmanageIDs = paramMap.userIds;
        var urlProjectmanageNames = paramMap.userNames;
        ...
        */
        ...
        findroomuserlist () {//查询项目子管理员
            ...
            if(_self.projectmanage.length==0){
                formdata.append("projectSn", projectSN)
                this.$http.post(getUrl() + "/pcontact_api/findprojectmanage", formdata).then(function (response) {
                    if (response.data.code == 0) {
                        var result = response.data.result;
        ...
        showAttment (ids) {//获取附件列表
            var _self=this;
            var params = new FormData();
            params.append("fileIdStr",ids);
            this.$http.post(getUrl() + "/sass_api/get_uploadfile_info", params).then(function (response) {
                if (response.data.code == 200) {
                    var result=response.data.result;
                    if(result.length>0){
        ...
        uploadImg () {//上传工程照片
            ...
            if(_self.fm.imgData){//更新工程照片和项目信息
                ...
                this.$http.post(getUrl() + "/pro_api/saveProImg", params).then(function (response) {
                    if (response.data.code == 200) {
                        //msg("xia");
                        setTimeout(function () {
