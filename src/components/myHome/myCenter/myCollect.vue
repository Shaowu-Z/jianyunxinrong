	...
	<section id="" class="mui-content">
		<div id="pullrefresh" class="mui-content mui-scroll-wrapper"  style="padding-top: 44px">
			<div class="mui-scroll">
				<div v-if="list.length>0" v-for="(obj,index) in list" :key="index">
	
						<div v-if="obj.type == 1">
							<div class="module01 openItem" :data-id="obj.productId" :data-type="obj.type" v-longtouch="timeOutEvent">
							<!--材料收藏-->
							<div class="module01-head header-media">
								<div class="user-info">
									<span class="user-name">材料</span>
								</div>
								<p class="secondary">{{obj.addTime | formDate}}</p>
							</div>
									...
									<div class="list-item">
										...
										<div class="product-info-box">
											...
											<div class="gray-pro-info"><span v-text="obj.buyNum"></span><span
													class="mui-pull-right" v-text="'已售' + obj.buyNum"></span></div>
										</div>
									</div>
								</div>
							</div>
						...
						<div v-else-if="obj.type == 3">
							<div class="module01 openItem" :data-id="obj.dishId" :data-type="obj.type"
								 :data-status="obj.status" :data-filename="obj.dishName" :data-url="obj.dishUrl"
								 :data-length="obj.dishSize" :data-thumb="obj.dishImg" v-longtouch="timeOutEvent">
								<!--云盘收藏-->
								<!--<div class="module01-head header-media">-->
									<!--<div class="user-info">-->
										<!--<img class="user-header"  v-bind:src="obj.dishUploaderIcon"/>-->
										<!--<span class="user-name" v-text="obj.dishUploader"></span>-->
									<!--</div>-->
									<!--<p class="secondary">{{obj.addTime | formDate}}</p>-->
								<!--</div>-->
								<!--<div class="module-body">-->
									<!--<div class="c-body" style="text-align: center">-->
										<!--<template v-if="obj.dishImg&&obj.dishImg!=''">-->
											<!--<img class="c-img"  v-bind:src="obj.dishImg">-->
										<!--</template>-->
										<!--<template v-else>-->
											<!--<img class="c-img"  src="../../../static/images/label-video.png">-->
										<!--</template>-->
										<!--<div class="c-content">-->
											<!--<div class="t-name mui-ellipsis-2" v-text="obj.dishName"></div>-->
											<!--<p class="secondary" v-text="obj.dishProjectName"></p>-->
										<!--</div>-->
									<!--</div>-->
								<!--</div>-->
						...
						<div v-else-if="obj.type == 5">
							<div class="module01 openItem" :data-id="obj.videoId" :data-type="obj.type"
								 :data-url="obj.videoUrl" :data-thumb="obj.videoImg"
	                             v-longtouch="timeOutEvent">
								<!--视频收藏-->
								<div class="module01-head header-media">
									<div class="user-info">
								...
								<div class="module-body">
									<div class="c-body" style="text-align: center">
										<div class="video-pic">
											<div v-if="obj.videoImg&&obj.videoImg!=''">
												<img style="height:100%" v-bind:src="obj.videoImg">
											</div>
											<div v-else>
												<img style="height:100%" src="../../../assets/images/detals.jpg">
											</div>
										</div>
									</div>
									<div v-if="obj.videoProjectName&&obj.videoProjectName!=''">
										<div class="c-footer">
						...
						<div v-else>
							<div class="module01 openItem" :data-id="obj.msgId" :data-type="obj.type"
								:data-url="obj.url" :data-msgType="obj.msgType" :data-thumb="obj.thumb"
								:data-msg="obj.msg" :data-addr="obj.addr" :data-lat="obj.lat"
								:data-lng="obj.lng" :data-filename="obj.filename" :data-msgId="obj.msgId"
								:data-addTime="obj.addTime" :data-length="obj.length" v-longtouch="timeOutEvent">
								<!--消息收藏-->
								<!--消息文字收藏-->
									...
									<div class="mui-card-content">
										<div class="mui-card-content-inner">
											...
											<p class="secondary">
												...
												<span v-if="obj.gourpNick!=''&&obj.gourpNick!=null">-</span>
												<span v-text="obj.fromUser"></span>
												<span id="addTime">{{obj.addTime | formDate}}</span>
											</p>
										</div>
									</div>
								</div>
								...
								<div class="c-box mui-card no-shadow" v-if="obj.msgType=='audio'">
									<div class="mui-card-content">
										<div class="mui-card-content-inner">
											...
											<div id="snd">
												<audio v-bind:src="obj.url" id="audio"></audio>
											</div>
								...
								<!--消息扩展消息收藏-->
								<!--<div class="c-box mui-card no-shadow" v-if="obj.msgType=='ext'">-->
									<!--<div class="mui-card-content">-->
										<!--<div class="mui-card-content-inner">-->
											<!--<div class="txt-content mui-ellipsis-3">-->
												<!--<a> {{obj.msg}}</a>-->
											<!--</div>-->
											<!--<p class="secondary">-->
												<!--<span v-text="obj.gourpNick"></span>-->
									...
									<div class="mui-card-content">
										<div class="mui-card-content-inner">
											<div class="article-content">
												...
												<div v-if="obj.todoContent!=null">
													<div v-for="(content,index) in splitContent(obj.todoContent)"	:key="index">
														<dl class="item">
		...
		<div class="mui-popup mui-popup-in" v-if="isSendCollect" id="send_collect" style="display:none;" >
			<div class="send-box">
	            ...
	            <div class="send-con" v-if="type==3">
	                ...
	                <div class="mui-ellipsis-2 mui-text-left" v-if="types==3">
	                   [文件]{{filename}}
	                 </div>
	            </div>
				<div class="send-con" v-else-if="type==6">
					<div class="mui-ellipsis-2 mui-text-left" v-if="msgType=='video'">
						<div class="img-content video-content" >
							<img v-if="thumb!=''" v-bind:src="thumb"/>
							<img v-else-if="thumb==''" src="../../../assets/images/detals.jpg">
						</div>
					</div>
...
export default {
  data () {
	...
	mounted() {
		...
		function collectClick(el) {
		    ...
		    document.getElementById("forwarding").onclick=function() {
		        ...
		        //转发
		        if(type==5||msgType=="video"||msgType=="txt"||msgType=="img"||msgType=="file"||msgType=="loc"||type==3) {
		            //获取转发的消息类型 1=图片 2=视频 3=文件 4=纯文本 5=位置
		            var types = 0;
		            if(type==5){
		                types = 2;
		            }else if(type==3){
		                types = this.istype(filename);
		            }else if(type==6){
		                if (msgType == "video") {
		                    types = 2;
		                } else if (msgType == "txt") {
	...
	created: function () {		
			...
			var _self = this;
	        if(this.$route.query.type!=null&&this.$route.query.type!=''&&this.$route.query.type!=undefined){
	        ...
	        //                         // console.log( pageParams.curPage+",sss"+allPage);
	        //                         mui('#pullrefresh').pullRefresh().endPullupToRefresh(( pageParams.curPage > allPage))
	        //                         this.loadData();
	        //                     },1500)
	        //                 }
	        //             }
			...
			// 	/*mui(".prolist-line").on('tap','.list-item',function() {
			// 		//获取id
			// 		var id = this.getAttribute("data-id");
			// 		this.openProduct(id);
			// 	})*/
	
			// 	mui("#this").on('tap','.openItem',function() {
	        //         var type = this.getAttribute("data-type");//收藏类型
	        //         var id = this.getAttribute("data-id");//收藏id
	        //         var url = this.getAttribute("data-url");//路径
	        //         var msgType = this.getAttribute("data-msgType");//消息类型
	        //         var thumb = this.getAttribute("data-thumb");//缩略图
	        //         var msg = this.getAttribute("data-msg");//消息内容
	        //         var addr = this.getAttribute("data-addr");//详细地址
	        //         var lat = this.getAttribute("data-lat");//纬度
	        //         var lng = this.getAttribute("data-lng");//经度
			// 		var filename = this.getAttribute("data-filename");//文件名
			// 		var addTime = this.getAttribute("data-addTime");//收藏时间
			// 		var length = this.getAttribute("data-length");//大小
			// 		/*alert(length);*/
	        //         //判断是要发送收藏，还是查看收藏
	        //         if(this.$data.isSendCollect){
			// 			//保存当前所点击收藏的一些信息
	        //             this.msg=msg;
	        ...
	        //                 this.types = this.istype(filename);
	        //             }
	        ...
	        //                       }
	        //                       //发送选取的内容
	        //                       /*this.sendCollect(note);*///废弃使用原生发送方式发送消息
			// 					  if(type==5){//以前的视频发送
	        //                           this.sendMessageNew(sendId,this.$data.message.img,this.$data.message.name
			...
			// });
	        setTimeout(function () {
	            _self.$data.listLength = _self.$data.list.length;
	        },600);
		},
					...
					loadData: function () {
						...
						this.$http.get("/api/collect/list", {
							...
							if (response.data.code == 0||response.data.code==200) {
								var rs = response.data;
								console.log(rs)
								allPage = rs.result.endPage;
								this.$data.result = rs.result;
								if (pageParams.curPage == 1) {
									//首次 加载一次
									this.$data.list = rs.result.list;
									mui('#pullrefresh').pullRefresh().endPulldownToRefresh();
									mui('#pullrefresh').pullRefresh().refresh(true);
								} else {
									this.$data.list = this.$data.list.concat(rs.result.list);
								}
			                    if (allPage==0){
