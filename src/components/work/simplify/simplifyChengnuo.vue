                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 总条数:<strong v-text="comm.formatNumber(item.tiao,0)"></strong></span>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.type=='双方承诺'?item.personAccept+' 与 '+item.personDistribute+' 的相互承诺':item.personAccept+' 向 '+item.personDistribute+' 的承诺'"></span>
                                                </div>
                                                <!--<p class="gray-pro-info sec-txt">承诺人：<strong class="mui-pull-right"  v-text=item.personAccept></strong></p>-->
                                                <!--<p class="gray-pro-info sec-txt">承诺单位：<strong class="mui-pull-right"  v-text="item.companySaleName"></strong></p>-->
                                                <p class="gray-pro-info sec-txt">承诺类型：<strong class="mui-pull-right"  v-text="item.type"></strong></p>
                                                <p class="gray-pro-info sec-txt">被承诺人：<strong class="mui-pull-right"  v-text="item.personDistribute"></strong></p>
                                                <p class="gray-pro-info sec-txt">被承诺单位：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
                                                <p class="gray-pro-info sec-txt">承诺内容：<strong class="mui-pull-right mui-ellipsis" style="display:block;width:60%;text-align: right;" v-text="item.beizhu"></strong></p>
                                                <p class="gray-pro-info sec-txt">承诺完成日期：<strong class="mui-pull-right"  v-text="item.dateChengnuo"></strong></p>
                                                <p class="gray-pro-info sec-txt">确认状态：<strong class="mui-pull-right"  v-text="item.confirmStatus==1?'已确认':'未确认'"></strong></p>
                                                <p class="gray-pro-info sec-txt">被承诺人评分：<strong class="mui-pull-right"  v-text="item.score==null?'未评分':item.score+'分'"></strong></p>
                                                <p class="gray-pro-info sec-txt" v-if="item.type=='双方承诺'">承诺人评分：<strong class="mui-pull-right"  v-text="item.threeScore==null||item.threeScore=='null'?'未评分':item.threeScore+'分'"></strong></p>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.companySaleRoomID,item.companySaleName],[item.companyBuyRoomID,item.companyBuyName],[item.personAcceptID,item.personAccept],[item.personDistributeID,item.personDistribute]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                ...
                                                <span class="name mui-ellipsis "  v-if ="group.No==2" v-text="item.personAccept" ></span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==3" v-text="item.personDistribute" ></span>
                                                <span class="mui-pull-right" v-text="'共计'+item.tiao+label.no_group_danwei"></span>
                                            </div>
                                            <p class="gray-pro-info sec-txt">总条数：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.tiao,0)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/chengnuo/transfer.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_chengnuo";
    $url.java_path_table = "/chart/column/table_w_chengnuo";
    // alert($url.java_path_table)
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["companySaleRoomID","companyBuyRoomID","personAcceptID","personDistributeID"];// 对应字段
    $label.group=["承诺单位分组","被承诺单位分组","承诺人分组","被承诺人分组"];// 配置分组标题
    $label.group_danwei=["个承诺单位","个被承诺单位","个承诺人","个被承诺人"]; // 分组后记录条数的单位
    $label.no_group_danwei="份"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["承诺日期","承诺完成日期"]; //排序的标题
    $order.by=["dateShenqing","dateChengnuo"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["承诺单位","被承诺单位","承诺人","被承诺人"];
    /**
     * 筛选页面
     */
    $label.sel_date=["承诺日期","承诺完成日期"];
    //筛选页面的选项的名称
    $label.sel=["承诺单位","被承诺单位","承诺人","被承诺人"];
    $search_list.used=["companySaleRoomID","companyBuyRoomID","personAcceptID","personDistributeID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["companySaleName","companyBuyName","personAccept","personDistribute"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["承诺单位列表","被承诺单位列表","承诺人列表","被承诺人列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
