                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 含税金额合计:<strong v-text="comm.formatNumber(item.money,2)"></strong></span>
                                        ...
                                        <div v-for="(item,index) in items_top" :key="index" style="display: block" >
                                            ...
                                            <div class="mui-table">
                                                <div class="mui-table-cell mui-text-center strong" >
                                                    <span class="num" v-text="comm.formatNumber(item.money,2)"></span>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.dateKaipiao+' '+comm.formatNumber(item.money,2)+'元'"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">不含税金额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.moneyOriginal,2)"></strong></p>
                                                <p class="gray-pro-info sec-txt">税额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.MoneyTax,2)"></strong></p>
                                                <p class="gray-pro-info sec-txt">发票代码：<strong class="mui-pull-right"  v-text="item.fapiaoCode"></strong></p>
                                                <p class="gray-pro-info sec-txt">发票号码：<strong class="mui-pull-right"  v-text="item.fapiaoHaoma"></strong></p>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.contractID,item.contractName],[item.companySaleRoomID,item.companySaleName],[item.companyBuyRoomID,item.companyBuyName]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                ...
                                                <span class="name mui-ellipsis "  v-if ="group.No==1" v-text ="item.companySaleName"></span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==2" v-text="item.companyBuyName" ></span>
                                                <!--<span class="name mui-ellipsis "  v-if ="$group.No==3" v-text="item.ContractType" ></span>-->
                                                <span class="mui-pull-right" v-text="'共计'+item.tiao+label.no_group_danwei"></span>
                                            </div>
                                            <p class="gray-pro-info sec-txt">含税金额合计：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.money,2)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/fapiao/transfer.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_fapiao_mingxi";
    $url.java_path_table = "/chart/column/table_w_fapiao_mingxi";
    /**
     * 主页页面
     */
    // 时间范围筛选
    $label.date_fanwei=["all",30,180,365] //v4 值域 ： all 3 7 30 60 90 180 365 thisWeek thisMonth thisSeason thisYear
    $param.default.whereDateType="dateKaipiao"// 默认时间范围的字段
    $param.default.whereDate="all"
    // 配置分组
    $group.by=["contractID","companySaleRoomID","companyBuyRoomID"];// 对应字段
    $label.group=["合同/编号分组","开票方分组","收票方分组"];// 配置分组标题
    $label.group_danwei=["个合同/编号","个开票方","个收票方"]; // 分组后记录条数的单位
    $label.no_group_danwei="份"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["开票日期","含税金额","不含税金额","税额"]; //排序的标题
    $order.by=["dateKaipiao","danjia","moneyOriginal","MoneyTax"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["合同/编号分组","开票方","收票方"];
    /**
     * 筛选页面
     */
    $label.sel_date=["开票日期"];
    //筛选页面的选项的名称
    $label.sel=[];
    $search_list.used=[];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=[]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=[];//查询列表页面的标题
    // 筛选页面的模糊查询项
    $label.sel_like = ["发票代码","发票号码"];//筛选页面的模糊查询标题
    $search.like=["fapiaoCode","fapiaoHaoma"] //筛选页面的模糊查询对应字段
    //筛选页面的区间查询的名称
    $label.sel_qujian_number=["含税金额"];
    $search.qujian_number=["danjia"];
    /**
     * //功能显示状态，可修改
     */
