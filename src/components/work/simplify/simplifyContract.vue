                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.contractName"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">含税金额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.htTotal,2)"></strong></p>
                                                <p class="gray-pro-info sec-txt">发票类型：<strong class="mui-pull-right"  v-text="item.fapiaoTitle"></strong></p>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.contractType,item.contractType],[item.companyBuyRoomID,item.companyBuyName],[item.companySaleRoomID,item.companySaleName],[item.fapiaoTitle,item.fapiaoTitle]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==0" v-text="item.contractType" ></span>
                                            ...
                                            <p class="gray-pro-info sec-txt">含税金额合计：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.htTotal,2)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_contract";
    $url.java_path_table = "/chart/column/table_w_contract";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["contractType","companyBuyRoomID","companySaleRoomID","fapiaoTitle"];// 对应字段
    $label.group=["业务类别分组","购买方分组","销售方分组","发票类型分组"];// 配置分组标题
    $label.group_danwei=["个类别","个购买方","个销售方","个类型"]; // 分组后记录条数的单位
    $label.no_group_danwei="份合同"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["签订日期"]; //排序的标题
    $order.by=["dateFasheng"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["类别","购买方","销售方","类型"];
    /**
     * 筛选页面
     */
    $label.sel_date=["签订日期"];
    //筛选页面的选项的名称
    $label.sel=["发票类型","购买方","销售方"];
    $search_list.used=["fapiaoTitle","companyBuyRoomID","companySaleRoomID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["fapiaoTitle","companyBuyName","companySaleName"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["发票类型列表","购买方列表","销售方列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
    $label.sel_like = ["名称"];//筛选页面的模糊查询标题
    $search.like=["contractName"] //筛选页面的模糊查询对应字段
    //筛选页面的区间查询的名称
    $label.sel_qujian_number=["含税金额"];
    $search.qujian_number=["htTotal"];
    /**
     * //功能显示状态，可修改
     */
