                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 会议纪要份数:<strong v-text="comm.formatNumber(item.tiao,0)"></strong></span>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                    ...
                                    <div v-for="(item,index) in items_recorder_list" :key="index" class="list-item" @click="divClick.onclick_recorder_list(item.id)">
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
												...
												<span class="name mui-ellipsis"  v-text="item.zhuchiPerson"></span>
											</div>
											<p class="gray-pro-info sec-txt">会议日期：<strong class="mui-pull-right"  v-text="item.dateFasheng"></strong></p>
											<p class="gray-pro-info sec-txt">主持人：<strong class="mui-pull-right"  v-text="item.zhuchiPerson"></strong></p>
											<p class="gray-pro-info sec-txt">参会人员：<strong class="mui-pull-right"  v-text="item.canhuiPerson"></strong></p>
											<p class="gray-pro-info sec-txt">确认状态：<strong class="mui-pull-right"  v-text="item.confirmStatus"></strong></p>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.zhuti,item.zhuti],[item.zhuchiPersonID,item.zhuchiPerson],[item.canhuiPersonID,item.canhuiPerson]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==0" v-text="item.zhuti" ></span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==1" v-text ="item.zhuchiPerson"></span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==2" v-text="item.canhuiPerson" ></span>
                                                <span class="mui-pull-right" v-text="'共计'+item.tiao+label.no_group_danwei"></span>
                                            </div>
                                            <p class="gray-pro-info sec-txt">>会议纪要份数：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.tiao,0)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/huiyijiyao/newInfo.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_huiyijiyao";
    $url.java_path_table = "/chart/column/table_w_huiyijiyao";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["zhuchiPersonID"];// 对应字段
    $label.group=["主持人分组"];// 配置分组标题
    $label.group_danwei=["个主持人"]; // 分组后记录条数的单位
    $label.no_group_danwei="份会议纪要"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["会议日期"]; //排序的标题
    $order.by=["dateFasheng"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["会议主题","主持人","参会人员"];
    /**
     * 筛选页面
     */
    $label.sel_date=["会议日期"];
    //筛选页面的选项的名称
    $label.sel=["会议主题","主持人","参会人员"];
    $search_list.used=["zhuti","zhuchiPersonID","canhuiPersonID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["zhuti","zhuchiPerson","canhuiPerson"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["会议主题列表","主持人列表","参会人员列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
