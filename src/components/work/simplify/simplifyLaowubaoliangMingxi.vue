                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.mingcheng+' '+(item.shuliang==null?0:item.shuliang)+'('+item.danwei+')'"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">报量单位：<strong class="mui-pull-right"  v-text="item.companySaleName"></strong></p>
                                                <p class="gray-pro-info sec-txt">报量日期：<strong class="mui-pull-right"  v-text="item.dateShenqing"></strong></p>
                                                <p class="gray-pro-info sec-txt">报量人：<strong class="mui-pull-right"  v-text="item.userName"></strong></p>
                                                <p class="gray-pro-info sec-txt">报量给：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.userID,item.userName]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==0" v-text="item.userName" ></span>
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/laowu/newInfo.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_laowubaoliang_mingxi";
    $url.java_path_table = "/chart/column/table_w_laowubaoliang_mingxi";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["userName"];// 对应字段
    $label.group=["工长分组"];// 配置分组标题
    $label.group_danwei=["个工长"]; // 分组后记录条数的单位
    $label.no_group_danwei="份"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["日期","数量","单价"]; //排序的标题
    $order.by=["dateShenqing","shuliang","danjia"]; //排序对应字段
    ...
    //筛选页面的选项的名称
    $label.sel=["工长"];
    $search_list.used=["userID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["userName"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["工长列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
    $label.sel_like = ["分项名称"];//筛选页面的模糊查询标题
    $search.like=["mingcheng"] //筛选页面的模糊查询对应字段
    //筛选页面的区间查询的名称
