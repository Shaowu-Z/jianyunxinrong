                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                               <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.name"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">日期：<strong class="mui-pull-right"  v-text="item.dateFasheng"></strong></p>
                                                <p class="gray-pro-info sec-txt">类别：<strong class="mui-pull-right"  v-text="item.type"></strong></p>
                                                <p class="gray-pro-info sec-txt">收件方：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.type,item.type],[item.room_id,item.room_name]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
											    ...
											    <span class="name mui-ellipsis "  v-if ="group.No==1" v-text ="item.room_name"></span>
                                            </div>
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/lianxi/transfer.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_gongzuolianxidan";
    $url.java_path_table = "/chart/column/table_w_gongzuolianxidan";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["type","room_id"];// 对应字段
    $label.group=["类别分组","收件公司分组"];// 配置分组标题
    $label.group_danwei=["个类别","个收件公司"]; // 分组后记录条数的单位
    $label.no_group_danwei="份"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["日期"]; //排序的标题
    $order.by=["dateFasheng"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["类别","收件公司"];
    /**
     * 筛选页面
     */
    $label.sel_date=["日期"];
    //筛选页面的选项的名称
    $label.sel=["类别","标题","收件方","发件方"];
    $search_list.used=["type","name","room_id","companySaleRoomID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["type","name","room_name","companySaleName"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["类别列表","标题列表","收件方列表","发件方列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
