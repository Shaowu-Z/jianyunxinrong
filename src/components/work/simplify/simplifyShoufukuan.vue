                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.dateFasheng+' '+comm.formatNumber(item.money,2)+'元'"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">合同/编号：<strong class="mui-pull-right"  v-text="item.contractName"></strong></p>
                                                <p class="gray-pro-info sec-txt">支出单位：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
                                                <p class="gray-pro-info sec-txt">收入单位：<strong class="mui-pull-right"  v-text="item.companySaleName"></strong></p>
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/shoufukuan/transfer.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_shoufukuan";
    $url.java_path_table = "/chart/column/table_w_shoufukuan";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    $label.group=["合同/编号分组","支出单位分组","收入单位分组"];// 配置分组标题
    $label.group_danwei=["个合同/编号","个支出单位","个收入单位"]; // 分组后记录条数的单位
    $label.no_group_danwei="份收付款"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["日期","金额"]; //排序的标题
    $order.by=["dateFasheng","money"]; //排序对应字段
    ...
    $label.title_search_list=["合同/编号列表","支出单位列表","收入单位列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
    $label.sel_like = [];//筛选页面的模糊查询标题
    $search.like=[] //筛选页面的模糊查询对应字段
    //筛选页面的区间查询的名称
    $label.sel_qujian_number=["金额"];
    $search.qujian_number=["money"];
    /**
     * //功能显示状态，可修改
     */
