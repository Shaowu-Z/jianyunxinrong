                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 金额合计:<strong v-text="comm.formatNumber(item.money,2)"></strong></span>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                    ...
                                    <div v-for="(item,index) in items_recorder_list" :key="index" class="list-item" @click="divClick.onclick_recorder_list(item.id)">
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                ...
                                                <p class="gray-pro-info sec-txt">到货日期：<strong class="mui-pull-right"  v-text="item.dateFasheng"></strong></p>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.companyBuyRoomID,item.companyBuyName],[item.companySaleRoomID,item.companySaleName],[item.pinpai,item.pinpai]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                <span class="name mui-ellipsis "  v-if ="group.No==0" v-text="item.companyBuyName" ></span>
                                            ...
                                            <p class="gray-pro-info sec-txt">金额合计：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.money,2)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_shouliao_mingxi";
    $url.java_path_table = "/chart/column/table_w_shouliao_mingxi";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["companyBuyRoomID","companySaleRoomID","pinpai"];// 对应字段
    $label.group=["购买方分组","销售方分组","品牌分组"];// 配置分组标题
    $label.group_danwei=["个购买方","个销售方","个品牌"]; // 分组后记录条数的单位
    $label.no_group_danwei="份";    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["收货日期","数量"]; //排序的标题
    $order.by=["dateFasheng","shuliang"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["购买方","销售方","品牌"];
    /**
     * 筛选页面
     */
    $label.sel_date=["收货日期"];
    //筛选页面的选项的名称
    $label.sel=["品牌","购买单位","销售单位"];
    ...
    $label.title_search_list=["品牌列表","购买单位列表","销售单位列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
