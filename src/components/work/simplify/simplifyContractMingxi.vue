                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 含税金额合计:<strong v-text="comm.formatNumber(item.htTotal,2)"></strong></span>
                                        ...
                                        <div v-for="(item,index) in items_top" :key="index" style="display: block" >
                                            ...
                                            <div class="mui-table">
                                                <div class="mui-table-cell mui-text-center strong" >
                                                    <span class="num" v-text="comm.formatNumber(item.htTotal,2)"></span>
											        <p class="sec-txt" >含税金额合计</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis" v-if="item.pinpai!=null"  v-text="item.mingcheng+'('+item.pinpai+')'+item.guigexinghao"></span>
                                                    <span class="name mui-ellipsis" v-else v-text="item.mingcheng+' '+item.guigexinghao"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">数量（单位）：<strong class="mui-pull-right"  v-text="item.shuliang+'('+item.danwei+')'"></strong></p>
                                                <p class="gray-pro-info sec-txt">单价：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.danjia,2)"></strong></p>
                                                <p class="gray-pro-info sec-txt">金额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.money,2)"></strong></p>
                                                <p class="gray-pro-info sec-txt">签订日期：<strong class="mui-pull-right"  v-text="item.dateFasheng"></strong></p>
                                                <p class="gray-pro-info sec-txt">购买方：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
                                                <p class="gray-pro-info sec-txt">销售方：<strong class="mui-pull-right"  v-text="item.companySaleName"></strong></p>
                                                </div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.companySaleRoomID,item.companySaleName],[item.companyBuyRoomID,item.companyBuyName],[item.pinpai,item.pinpai]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
                                                ...
                                                <span class="name mui-ellipsis "  v-if ="group.No==2" v-text="item.pinpai" ></span>
                                                <!--<span class="name mui-ellipsis "  v-if ="$group.No==3" v-text="item.fapiaoTitle" ></span>-->
                                                <span class="mui-pull-right" v-text="'共计'+item.tiao+label.no_group_danwei"></span>
                                            </div>
                                            <p class="gray-pro-info sec-txt">含税金额合计：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.money,2)">222</strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/hetong/transfer.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_contract_mingxi";
    $url.java_path_table = "/chart/column/table_w_contract_mingxi";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["companySaleRoomID","companyBuyRoomID","pinpai"];// 对应字段
    $label.group=["销售方分组","购买方分组","品牌分组"];// 配置分组标题
    $label.group_danwei=["个销售方","个购买方","个品牌"]; // 分组后记录条数的单位
    $label.no_group_danwei="份"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["签订日期","数量","单价","金额"]; //排序的标题
    $order.by=["dateFasheng","shuliang","danjia","money"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["销售方","购买方","品牌"];
    /**
     * 筛选页面
     */
    $label.sel_date=["签订日期"];
    //筛选页面的选项的名称
    $label.sel=["品牌","购买方","销售方"];
    $search_list.used=["pinpai","companyBuyRoomID","companySaleRoomID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["pinpai","companyBuyName","companySaleName"];// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["品牌列表","购买方列表","销售方列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
    $label.sel_like = ["名称","规格"];//筛选页面的模糊查询标题
    $search.like=["mingcheng","guigexinghao"]; //筛选页面的模糊查询对应字段
    //筛选页面的区间查询的名称
    $label.sel_qujian_number=["数量","单价","金额"];
    $search.qujian_number=["shuliang","danjia","money"];
    /**
     * //功能显示状态，可修改
     */
