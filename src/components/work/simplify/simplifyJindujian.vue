                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 进度资料条数:<strong v-text="comm.formatNumber(item.tiao,0)"></strong></span>
                                        ...
                                        <a class="" href="javascript:void(0)">
                                            <div class="product-info-box text">
                                                <div class="labor-name">
                                                    ...
                                                    <span class="name mui-ellipsis"  v-text="item.type"></span>
                                                </div>
                                                <p class="gray-pro-info sec-txt">发生日期：<strong class="mui-pull-right"  v-text="item.dateFasheng"></strong></p>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.type,item.type]])">
                                        <div class="product-info-box">
                                            ...
                                            <p class="gray-pro-info sec-txt">进度资料条数：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.tiao,0)"></strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    $url.module_path=["/static/newwebstatic/jindu/newInfo.html?id="];// 这个根据不同的业务模块需要更改；
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_jindujian";
    $url.java_path_table = "/chart/column/table_w_jindujian";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    // 配置分组
    $group.by=["type"];// 对应字段
    $label.group=["文件类别分组"];// 配置分组标题
    $label.group_danwei=["个类别"]; // 分组后记录条数的单位
    $label.no_group_danwei="份付款明细"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["文件类别","发生日期"]; //排序的标题
    $order.by=["type","dateFasheng"]; //排序对应字段
    ...
    //筛选页面的选项的名称
    $label.sel=["文件类别"];
    $search_list.used=["type"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["type"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["文件类别列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
