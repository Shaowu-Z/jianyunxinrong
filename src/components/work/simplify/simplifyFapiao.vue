                            ...
                            <div id="id_Top" class="gather-item fixed-top">
                                ...
                                <div id="id_mini_top" class="gather-item-header tight" style="display: none;background: #eee;" >
                                    <div v-for="(item,index) in items_top" :key="index">
                                        <span class="mui-pull-right"> 含税金额合计:<strong v-text="comm.formatNumber(item.moneyTotal,2)"></strong></span>
                        ...
                        <div id="id_list" class="mui-scroll-wrapper"  style="padding-top:0;display: none;position:initial;">
                            <div id="mui-scroll" class="mui-scroll" style="position:initial;">
                                ...
                                <div id="recorder_list" class="prolist-line project-list cus_data" style="display: none">
                                    <div v-for="(item,index) in items_recorder_list" :key="index" class="list-item" @click="divClick.onclick_recorder_list(item.id)">
                                        <a class="" href="javascript:void(0)">
											...
											<p class="gray-pro-info sec-txt">开票单位：<strong class="mui-pull-right"  v-text="item.companySaleName"></strong></p>
											<p class="gray-pro-info sec-txt">收票单位：<strong class="mui-pull-right"  v-text="item.companyBuyName"></strong></p>
											<p class="gray-pro-info sec-txt">无税金额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.originalTotal,2)+'元'"></strong></p>
											<p class="gray-pro-info sec-txt">税额：<strong class="mui-pull-right"  v-text="comm.formatNumber(item.taxTotal,2)+'元'"></strong></p>
                                ...
                                <div id="group_list" class="prolist-line project-list cus_data" >
                                    <div v-for="(item,index) in litems_group_list" :key="index" class="list-item" @click="divClick.group_list([[item.type,item.type]])">
                                        <div class="product-info-box">
                                            <div class="labor-name" ><span class="list-num" >{{index+1}}.</span>
											<span class="name mui-ellipsis "  v-if ="group.No==0" v-text="item.type" ></span>
											<span class="mui-pull-right" v-text="'共计'+item.tiao+label.no_group_danwei"></span>
										</div>
										<p class="gray-pro-info sec-txt">含税金额合计：<strong class="mui-pull-right" style="font-size: 17px;" v-text="comm.formatNumber(item.moneyTotal,2)">222</strong></p>
                                        </div>
                                    </div>
                                </div>
                                <!--group list -end- -->
...
<script>
    ...
    // js链接java的链接配置
    $url.java_path_group = "/chart/column/group_w_fapiao";
    $url.java_path_table = "/chart/column/table_w_fapiao";
    /**
     * 主页页面
     */
    // 时间范围筛选
    ...
    $label.group=["合同/编号分组","开票单位分组","收票单位分组"];// 配置分组标题
    $label.group_danwei=["个合同/编号","个开票单位","个收票单位"]; // 分组后记录条数的单位
    $label.no_group_danwei="个发票"    //无汇总的情况下，记录条数的单位
    //配置排序
    $label.order=["日期","含税金额","税额"]; //排序的标题
    $order.by=["dateFasheng","moneyTotal","taxTotal"]; //排序对应字段
    $label.order_default=$label.order[0]; //默认排序
    $param.default.orderby=$order.by[0];   // 默认排序的标题
    //条件显示框里的名称
    $label.tiaojian=["合同/编号","开票单位","收票单位"];
    /**
     * 筛选页面
     */
    $label.sel_date=["日期"];
    //筛选页面的选项的名称
    $label.sel=["合同/编号","开票单位","收票单位"];
    $search_list.used=["contractID","companySaleRoomID","companyBuyRoomID"];// 查询列表的名称，要和筛选页面的选项的名称对应
    $search_list.name=["contractName","companySaleName","companyBuyName"]// 查询列表的name字段名，要$search_list.used[]对应
    $label.title_search_list=["合同/编号列表","开票单位列表","收票单位列表"];//查询列表页面的标题
    // 筛选页面的模糊查询项
