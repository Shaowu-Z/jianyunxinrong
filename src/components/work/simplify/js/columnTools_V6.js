 ...
 * 版本号 v6
 * 功能与事件的实现方法
 * 2017-11-29 修改了，筛选页面的重置功能
 * 2017-11-30
 *          日期范围增加 本周末 本月末 本季度末 本年末
 * 2017-12-2
 *          筛选页面的条件选项的条件列表，升级为自动生成
 * 2017-12-5
 *          筛选页面的条件选项的条件列表，增加了模糊搜索
 * 2017-12-12
 *          筛选页面的条件选项增加了预警等非字段的筛选
 */
// 只有手机上，可以下来刷新；
...
var $url={
    ...
    get_recorder_list:function (val) {
        var u
        switch (val){
            case "haosi":
                u=HAOSI_SERVER_ADDRESS+$url.module_path+"!open.mdo?__SESSIONID_="+getCookie("sessionid")+"&id="
                break;
            case "sunline":
                u=getUrl()+$url.module_path
                break;
        }
        return u
    },
...
var sel_date =  function (t) {
    if(t=="s"){
        if($date.sel_dateStart != "请选择"){
            opt.value = $date.sel_dateStart;
        }
    }else if(t==='e'){
        if($date.sel_dateEnd != "请选择"){
            opt.value = $date.sel_dateEnd;
        }
    }
    var picker = new mui.DtPicker(opt);
    picker.show(function (rs) {
        if(t=="s"){
            if(new Date(rs.value) > new Date($date.sel_dateEnd)){
                layer.open({
                    content: '不能小于开始日期'
                    ,skin: 'msg'
                    ,time: 2 //2秒后自动关闭
            ...
            $date.sel_dateStart = rs.value;

            // app.sel_dateStart = rs.value;
            ...
            if(new Date($date.sel_dateStart) > new Date(rs.value)){
                layer.open({
                    content: '不能小于开始日期'
                    ,skin: 'msg'
                    ,time: 2 //2秒后自动关闭
            ...
            $date.sel_dateEnd = rs.value;
            // app.sel_dateEnd = rs.value;
...
var $date ={
    ...
    get_date_fanwei:function (days,used) {
        ...
        if(days=="all" || days=="全部" ){
            ...
            switch (days) {
                case "本年":
                case "thisYear":
                    $date.sel_dateEnd = formatDate(date, "yyyy-mm-dd");
                ...
                default:
                    if (days != "") {
                        $date.sel_dateEnd = formatDate(date, "yyyy-mm-dd");
                        $date.sel_dateStart = formatDate($date.before(date, parseInt(days)), "yyyy-mm-dd");
                    }
                    break;
            }
        }
    ...
    date_saixuaned:function(days,id,p_id,className) {
        ...
        if(days=="all" || days=="全部" ){
            ...
            // app.date_qujian = $date.group_top("","");
            // 主页面时间范围
            ...
            // if(document.getElementById("sel_dateStart")){
            //     document.getElementById("sel_dateStart").innerHTML="-";
            //     document.getElementById("sel_dateStart").style.display="block"
            // }
            // document.getElementById("val_del_dateStart").style.display="none"
            // if(document.getElementById("sel_dateEnd")){
            //     document.getElementById("sel_dateEnd").innerHTML="-";
            //     document.getElementById("sel_dateEnd").style.display="block"
            // }
            // document.getElementById("val_del_dateEnd").style.display="none"
            // app.sel_dateStart="-";
            // // app.sel_dateEnd=formatDate(myDate.toLocaleDateString(),"yyyy-mm-dd");
            // app.sel_dateEnd="-"
        }else{
            // 主页面时间范围
            ...
            // app.sel_dateStart=$date.sel_dateStart;
            // app.sel_dateEnd=$date.sel_dateEnd;
        }
    },
    /**
     * 日期范围筛选，用于查询主页
        ...
        if(whereDateStart!="" ){
            if(whereDateEnd !=""){
                if(whereDateStart=="-"&&whereDateEnd=="-"){
                    ...
                    try{
                        return formatDate(whereDateStart,"yyyy-mm-dd")+" 至 "+ formatDate(whereDateEnd,"yyyy-mm-dd");
                    }catch (e) {
                ...
                try{
                    return formatDate(whereDateStart,"yyyy-mm-dd") +" 之后";
                }catch (e) {
            ...
            if(whereDateEnd !=""){
                try{
                    return formatDate(whereDateEnd,"yyyy-mm-dd") +" 之前";
                }catch (e) {
...
var $label={
    ...
    set:{
        title:function () {
            // 设置标题，固定写法
            document.getElementById("title1").innerHTML= $param.windowTitle;
            // document.getElementById("title").innerHTML= $param.windowTitle;
        } ,
        /**
         *设置分组项内容
         */
        ...
        //         if(document.getElementById("sel_label"+i))
        //             document.getElementById("sel_label"+i).innerHTML= $label.sel[i];
        //     }
        // },
        /**
         * 设置分组的名称
...
var $param ={
    ...
    url:function () {
        var param =""
        param = param + ("homepage_url="+getUrl());
        ...
        param = param + ("&groupby=" + $param.groupby );
        // if ($param.groupClicked.indexOf($param.groupby) == -1) {
        //     $param.groupClicked=$param.groupClicked+"_"+ $param.groupby;
        //     $param.groupListClicked = $param.groupListClicked+"_"+ $param.groupby;
        // }
        // param = param + ("&groupClicked=" + $param.groupClicked);
        // param = param + ("&groupListClicked=" + $param.groupListClicked);
        ...
        param = param + ("&__SESSIONID_=" + getCookie("sessionid"));
        return param;
    },
    /**
     * 调试用，显示参数具体内容
...
var $data ={
        ...
        switch (used){
            ...
            case "select_ok": // 点击筛选的确定
                if($show_status.gongneng.need_top_show==true){
                    // if($show_status.gongneng.top_show ==true)
                    $data.get_data("top",$url.java_path_group);
                }
            case "zhankaishouqi": // 点击展开收起
            case "group": //点击分组
            case "orderby":// 点击排序
                // if($id_show.list==true){
                // $id_show.get_isShow("get_all_data  ");
                $data.get_list_data();
                // }
                break;
        }
        ...
        switch (use){
            ...
            case "recorder_list":
                if(array==null || array.length==0){
                    ...
                    app.$data.items_recorder_list = array;
                    // 数据条数
            ...
            case "group_list":
                if(array==null || array.length==0){
                    ...
                    app.$data.litems_group_list = array;
                    // 数据条数
            ...
            if(array==null || array.length==0){
                ...
                switch (use){
                    case "top":
                        app.$data.items_top = array;
                        //判断本次读取是否有数据
                        $data.have.top_thisTime= app.$data.items_top[0].tiao==0?false:true;
    ...
    get_data_search_list:function(No,id){
        ...
        if(No<$label.sel.length) {
            ...
            var path = getUrl()+ $url.java_path_group;
            // 从java获得数据
            axios.post(path, param).then(function (response) {
//					console.info(used);
                console.info(response.data.result);
...
var $onclick={
    ...
    //     var param = $param.url();
    //     // alert(param)
    //     var encodeData = window.btoa(window.encodeURIComponent(param))//编码
    //     url += encodeData;
    ...
    inputNew:function () {
        ...
        if($param.inputUrl != "" ){
            ...
            //     url = url + "!new.mdo?";
            url = url +"?";
            url = url + "&__SESSIONID_="+getCookie("sessionid");
    ...
    orderby:function (No,id,p_id) {
        ...
        $data.get_list_data();
        // 调整页面
        // $zhankaiShouqi.tiaojianlan($show_status.tiaojianlan(app.$data.t_val));
    ...
    number_status:function(status){
        ...
        // 给获取列表数据
        if(app.$data.recorder_list_show== true)
            $data.get_data("recorder_list",$url.java_path_table);
        $param.whereStatus = status_temp;
        //取消分组时，点击金额改变样式
        if(app.recorder_list_show==true)
            money_chang("#4ba9e9",status);
    },
    // 点击页面列表的点击事件===========================================================================================
    /**
     * 有　业务模块
    ...
    onclick_recorder_list:function (recorderID,module_path) {
        console.info("recorderID="+recorderID);
        console.info("recorderID_last="+$onclick.recorderID_last);
        var child_url = $url.get_recorder_list($url.recorder_type) + recorderID
        // alert(child_url);
    ...
    group_list:function (val) {
        ...
        // $onclick._zhankaishouqi_zhankai("group_top")
        // $div_status.top.open("group_top");
        ...
        $tiaojian.name_short[i]= $formatString($tiaojian.name_long[i],3);
        ...
        // app.$data.t_val[i] = $tiaojian.name_long[i];
        // app.$data.sel[i] = $tiaojian.name_long[i];
        ...
        // 获取顶部汇总数据
        // $show_status.list_groupOrRecorder("list_recorder");
        ...
        // 条件栏页面
        // $tiaojian_change.show($tiaojian_change.isShow(app.$data.t_val));
        $tiaojian_change.show($tiaojian_change.isShow($tiaojian.name_long));
    ...
    select_ok:function(){
        ...
        if($show_status.gongneng.sel_date==true){
            if($date.sel_dateStart_last!=$date.sel_dateStart || $date.sel_dateEnd_last!=$date.sel_dateEnd){// 发生变化了
            // if($param.whereDateStart!=$date.sel_dateStart || $param.whereDateEnd!=$date.sel_dateEnd){// 发生变化了
                // if($date.sel_dateStart!="-" && $date.sel_dateEnd =="-"){
                //     if($param.whereDateStart!=$date.sel_dateStart || $param.whereDateEnd!=$date.sel_dateEnd){
                //         document.getElementById("id_saixuan").innerHTML="自定义";
                //         $onclick.onclicked.clear_selected("saixuan_li"," active")
                //     }
                // }else if($date.sel_dateStart=="-" && $date.sel_dateEnd =="-"){
                //     document.getElementById("id_saixuan").innerHTML="自定义";
                //     $onclick.onclicked.clear_selected("saixuan_li"," active")
                // }
                //功能栏标题改为自定义
    ...
    select_reset:function () {
        ...
        if($show_status.gongneng.ul_search_combox==true){
            ...
            for(var i=0;i<j;i++){
                // app.sel[i]=$tiaojian.name_long[i]==""||$tiaojian.name_long[i]==undefined?"请选择":$tiaojian.name_long[i];
        ...
        if($show_status.gongneng.fuzzy_query==true){
            for(var i=0;i<$search.like.length;i++){
                if(document.getElementById("sel_like_input"+i))
                    // document.getElementById("sel_like_input"+i).value=$search.last.like[i];
    ...
    search_list_open:function (id,No) {
        ...
        $search.list_No=No
        // app.search_list_No=$search.list_No;
        ...
        setTimeout(function () {
            ...
            if($search_list.No_val[No] == undefined){
                ...
                // alert(No+"====="+ $label.sel.length)
                $search_list.input_changed(1)
                // $data.get_data_search_list(No,id);
    ...
    set_selected_val:function (No, name, sn) {
        // app.$data.sel[No]= name==""?"请选择":name;
        $search.list_name_selected = name==""?"请选择":name;


        $search.selected_Name[No]=name==""?"请选择":name;
        $search.selected_Name_short[No]= $formatString(name,3)
...
// /**
//  *
//  * @type {{fenzu_ul: $saixuan.fenzu_ul, xuanzhe: $saixuan.xuanzhe}}
//  */
// var $saixuan={
//     fenzu_ul:function (ul_id,li_id,li_class,li_value) {
...
//      *  选择后,修改class 样式
//      */
//     xuanzhe:function (c_id,p_id,className) {
//         // 删除被选择标记
//         var doclist = document.getElementById(p_id).childNodes;
//         for (var i=0;i<doclist.length;i++){
//             if(doclist[i].className && doclist[i].className.indexOf(className)!=-1){
//                 doclist[i].className = doclist[i].className.replace(className,"");
//             }
//         }
//         // 选择后,被选择的修改className
//         document.getElementById(c_id).className = document.getElementById(c_id).className + className;
//     }
// }
// 修改选择标签颜色-------------------------------------------------------------------------------------------------
function money_chang(color,nums,decide,classname){
    //获取所有数据对象
