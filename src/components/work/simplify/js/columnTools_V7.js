...
var $url={
    ...
    get_recorder_list:function (url_type,yewu_name,chanshus) {
        ...
        switch (url_type){
            case "haosi":
                u=HAOSI_SERVER_ADDRESS+path+"!open.mdo?__SESSIONID_="+getCookie("sessionid")+"&id="
                break;
            case "sunline":
                u=getUrl()+path;
                break;
...
var $date ={
    ...
    get_now:function () {
        var date = new Date();
        $date.yy_mm= formatDate(date,"yyyy-mm")
...
var $onclick={
    ...
    group:function (groupNo,id,p_id,className) {
        ...
        if(groupNo == '无分组'){ //取消分组，或者无分组
            ...
            if(app.items_recorder_list.length==0)
    ...
    onclick_recorder_list:function (recorderID,yewu_name,chanshus) {
        ...
        var child_url = $url.get_recorder_list($url.recorder_type,yewu_name,chanshus) + recorderID
        // alert(child_url);
